...
const TenderDetails = () => {
  ...
  const renderCategory = (category) => {
    ...
    return (
      <li key={category.id} style={{ marginLeft: `${indentLevel * 20}px` }} className="my-1">
        {indentLevel > 0 && <span className="text-gray-400">└─ </span>}
        <a 
          href={category.link} 
          target="_blank" 
  ...
  <tbody>
    {Object.entries(tender).map(([key, value]) => {
      if (typeof value === "string" && 
          ...
          key !== "source_url" &&
          key !== "title" &&
          key !== "Region URL") {  // Added this condition to exclude Region URL
        return (
      ...
      {tender.categories && tender.categories.length > 0 && (
        <div className="mt-6">
          ...
          <ul className="list-none bg-gray-50 p-4 rounded-lg border">
            {tender.categories
